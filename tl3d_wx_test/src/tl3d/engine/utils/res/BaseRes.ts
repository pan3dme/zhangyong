module tl3d {
    export class BaseRes extends ResCount {
        ...
        //读取浮点数据，两个字节
        public static readFloatTwoByte(byte: TLByteArray, vertices: Array<number>): void {
        ...
        //读取一个字节的LightMap
        public static readFloatOneByte(byte: TLByteArray, vertices: Array<number>): void {
            var verLength: number = byte.readInt();
            if (verLength > 0) {
                for (var i: number = 0; i < verLength; i++) {
        ...
        public static readIntForTwoByte(byte: TLByteArray, indexs: Array<number>): void {
        ...
        public static readIntForOneByte(byte: TLByteArray, indexs: Array<number>): void {
            var iLen: number = byte.readInt();
            for (var i: number = 0; i < iLen; i++) {
         ...
         * 2 readIntForOneByte
         *  */
        public static readBytes2ArrayBuffer($byte: TLByteArray, $data: DataView, $dataWidth: number, $offset: number, $stride: number, $readType: number = 0): void {
            var verLength: number = $byte.readInt();
        ...
        //读取材质参数
        public static readMaterialParamData(byte: TLByteArray): Array<any> {
