...
module tl3d {
    ...
    export  class TLByteArray {
        ...
        public setdata(srcByte: TLByteArray): void {
         ...
         */
        public readBytes(bytes: TLByteArray, offset: number = 0, length: number = 0): void {
            //if (length == 0) {
         ...
         */
        public writeBytes(bytes: TLByteArray, offset: number = 0, length: number = 0): void {
        ...
        public writeString(value: string): void {
            var strByteArray: TLByteArray = new TLByteArray();
        ...
        public readVector3D($w: boolean = false): tl3d.Vector3D {
            var $p: tl3d.Vector3D = new tl3d.Vector3D;
            $p.x = this.readFloat()
            $p.y = this.readFloat()
            $p.z = this.readFloat()
            if ($w) {
