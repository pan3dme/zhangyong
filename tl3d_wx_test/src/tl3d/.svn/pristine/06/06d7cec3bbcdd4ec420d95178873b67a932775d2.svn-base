module tl3d {
    export class Skill extends ResCount {
        protected skillVo: SkillVo; //技能编辑器配置信息
        public name: string;
        public key: string;
        public isDeath: boolean = true;
        public keyAry: Array<SkillKey>; //技能帧信息
        ...
        public trajectoryAry: Array<SkillTrajectory>; //技能粒子运动规则对象（多个目标就有多个）
        ...
        public needShock: boolean = false;
        ...
        public setKeyAry(): void {
            ...
            if (this.skillVo.types == SkillType.FixEffect) {
                for (var i: number = 0; i < this.skillVo.keyAry.length; i++) {
                    var keySkill: SkillFixEffect = new SkillFixEffect(this);
                ...
                for (var i: number = 0; i < this.skillVo.keyAry.length; i++) {
                    ...
                    if (tkv.multype == 1) {
                        trajectory = new SkillMulTrajectory(this.skillMgr);
                    } else {
                        trajectory = new SkillTrajectory(this.skillMgr);
                    }
        ...
        private getSound(): void {
            ...
            if (this.skillVo.sound.frame < this.time) {
                AudioMgr.playSound("skill/sound/"+this.skillVo.sound.url);
                this.soundPlay = true;
            }
