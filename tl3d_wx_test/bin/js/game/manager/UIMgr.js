...
var UI_DEPATH;
(function (UI_DEPATH) {
    UI_DEPATH[UI_DEPATH["BOTTOM"] = 0] = "BOTTOM";
    UI_DEPATH[UI_DEPATH["MIDDLE"] = 1] = "MIDDLE";
    UI_DEPATH[UI_DEPATH["TOP"] = 2] = "TOP";
    UI_DEPATH[UI_DEPATH["GUIDE"] = 3] = "GUIDE";
    UI_DEPATH[UI_DEPATH["LOADING"] = 4] = "LOADING";
    UI_DEPATH[UI_DEPATH["WAITING"] = 5] = "WAITING";
    UI_DEPATH[UI_DEPATH["ALERT"] = 6] = "ALERT";
})(UI_DEPATH || (UI_DEPATH = {}));
var UI_DEPATH_VALUE;
...
})(UI_DEPATH_VALUE || (UI_DEPATH_VALUE = {}));
var UIMgr = /** @class */ (function () {
    ...
    //注册ui
    UIMgr.prototype.registerUI = function (ui, uiname, atlases, destroyAtlases, depth, popEffect, isModal, closeOther, parentUI, isQueue, adaptStage) {
        if (popEffect === void 0) { popEffect = false; }
        if (isModal === void 0) { isModal = false; }
        if (closeOther === void 0) { closeOther = false; }
        if (parentUI === void 0) { parentUI = null; }
        if (isQueue === void 0) { isQueue = false; }
        if (adaptStage === void 0) { adaptStage = false; }
        if (!this._uiregist[uiname]) {
            var info = { uiname: uiname, cls: ui, atlases: atlases, depth: depth, popEffect: popEffect, isModal: isModal, closeOther: closeOther, parentUI: parentUI, destroyAtlases: destroyAtlases, isQueue: isQueue, adaptStage: adaptStage };
     ...
     * 显示过场加载
     */
    UIMgr.prototype.showLoading = function (text) {
        logyhj("显示load");
        if (!this._loadui) {
    ...
    Object.defineProperty(UIMgr.prototype, "loadui", {
        get: function () {
            return this._loadui;
        },
        enumerable: true,
     ...
     * @param value
     */
    UIMgr.prototype.loadingProcess = function (value, text) {
        if (this._loadui) {
            if (text) {
                this._loadui.dataSource = text;
            }
     ...
     * 显示加载转圈
     */
    UIMgr.prototype.showWaiting = function (text) {
        if (!this._waitui) {
     ...
     * @param value
     */
    UIMgr.prototype.waitingProgress = function (value) {
        if (this._waitui) {
     ...
     * 隐藏加载转圈
     */
    UIMgr.prototype.hideWaiting = function () {
        if (this._waitui) {
            this._waitui.close(null, false, false);
        }
    };
    UIMgr.prototype.getUIInfo = function (uiname) {
        return this._uiregist[uiname];
    };
    /**显示ui */
    UIMgr.prototype.showUI = function (uiname, dataSource, sound, preinit) {
        var _this = this;
        if (sound === void 0) { sound = true; }
        if (preinit === void 0) { preinit = false; }
        //创建
        var uiinfo = this._uiregist[uiname];
        if (!uiinfo) {
        ...
        var self = this;
        var dialogExt = this._uicache[uiname];
        if (!dialogExt) {
            //如果需要预加载资源
            if (uiinfo.atlases || uiinfo.destroyAtlases) {
                //开始加载ui
                var atlases = GameUtil.concatArray(uiinfo.atlases, uiinfo.destroyAtlases);
                if (this._loadingResUi.indexOf(uiname) == -1) {
                    this._loadingResUi.push(uiname);
                    this.showWaiting();
                    Laya.loader.load(atlases, Handler.create(null, function ($dataSource, result) {
                        // logyhj("加载结果：",result);
                        if (result === false) {
                            _this.delLoadingUi(uiname);
                            //资源加载出错，就不打开界面
                        ...
                        _this.catchCom(dialogExt, uiinfo, $dataSource, preinit);
                    }, [dataSource]));
                }
            }
            else {
    ...
    /** 删除正在加载的ui */
    UIMgr.prototype.delLoadingUi = function (uiname) {
        var index = this._loadingResUi.indexOf(uiname);
        if (index != -1) {
     ...
     * @param fun
     */
    UIMgr.prototype.catchCom = function (dialog, uiinfo, dataSource, preinit) {
        if (preinit === void 0) { preinit = false; }
        if (!dialog) {
            logerror("uierror");
    ...
    /**隐藏ui */
    UIMgr.prototype.hideUIByName = function (uiname, showEffect) {
        var cui = this._uicache[uiname];
     ...
     */
    UIMgr.prototype.hideUIByDepth = function (depths, excludes) {
        if (excludes === void 0) { excludes = []; }
        for (var _i = 0, depths_1 = depths; _i < depths_1.length; _i++) {
            var depth = depths_1[_i];
            var len = DialogExt.manager.numChildren;
            for (var i = len - 1; i >= 0; i--) {
                var dialog = DialogExt.manager.getChildAt(i);
    ...
    /**获取ui */
    UIMgr.prototype.getUIByName = function (uiname) {
        return this._uicache[uiname];
    };
    UIMgr.prototype.hasStage = function (uiname) {
        var cui = this._uicache[uiname];
        if (cui && cui.parent)
            return true;
        else
    ...
    /** 舞台上是否有该ui组 */
    UIMgr.prototype.hasStageByGroup = function (uigroup) {
        var group = DialogExt.manager.getDialogsByGroup(uigroup);
        return group && group.length > 0;
    };
    UIMgr.prototype.clearUICache = function () {
        this._uicache = {};
    };
    /**显示ui */
    UIMgr.showUI = function (uiname, dataSource, sound, preinit) {
        if (sound === void 0) { sound = true; }
        if (preinit === void 0) { preinit = false; }
        UIMgr.getInstance().showUI(uiname, dataSource, sound, preinit);
    };
    /**隐藏ui */
    UIMgr.hideUIByName = function (uiname, showEffect) {
        UIMgr.getInstance().hideUIByName(uiname, showEffect);
    };
    UIMgr.getUIByName = function (uiname) {
        return UIMgr.getInstance().getUIByName(uiname);
    };
    UIMgr.hasStage = function (uiname) {
