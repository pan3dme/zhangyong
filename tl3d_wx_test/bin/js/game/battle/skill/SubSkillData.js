var battle;
(function (battle) {
    var SubSkillData = /** @class */ (function () {
        // private mainTarget: BattleObj;
        function SubSkillData(skillId, subSkillId) {
            //选择友方的类型字段
            this._selectKeys = [iface.tb_prop.targetTypeKey.ownMinHp,
        ...
        SubSkillData.prototype.addUseNum = function () {
            var tb_skill = tb.TB_skill.get_TB_skillById(this.skillId);
        ...
        SubSkillData.prototype.checkUseNum = function () {
            return this.useNum < this.useNumMax;
        };
        SubSkillData.prototype.getTargetIds = function () {
            var ids = [];
        ...
        SubSkillData.prototype.genSkillTargets = function (skillobj, atker, extTarget) {
            if (!this.checkUseNum()) {
            ...
            if (!skillobj.isPassive() && this._selectKeys.indexOf(this.targetType) == -1) {
                ...
                var stBackTarget = atker.getStBackTarget();
                if (stBackTarget) {
                    //有反击对象
                    ...
                    var sneerTarget = atker.getSneerTarget();
                    if (sneerTarget) {
                        //有嘲讽对象
            ...
            switch (this.targetType) {
                ...
                case iface.tb_prop.targetTypeKey.frontAndRandomBack:
                    var rowObjs = atker.getEnemyRowObjs(1);
        ...
        //     }
        // }
        SubSkillData.prototype.clearTmpAmend = function () {
            if (!this.checkUseNum()) {
                return;
            }
            for (var i = 0; i < this.targets.length; i++) {
         ...
         * @param atker
         */
        SubSkillData.prototype.calEffectHits = function (atker, isPassive) {
            this.effectHits = {};
            for (var i = 0; i < this.targets.length; i++) {
                var target = this.targets[i];
                var bHit = true;
        ...
        SubSkillData.prototype.doEffects = function (skillInfo, triggerType, atker, prtSkillId, extParam) {
            if (!this.checkUseNum()) {
                return;
            }
        ...
        SubSkillData.prototype.canTriggerSkillEff = function (triggerType) {
            var result = false;
            for (var i = 0; i < this.triggerTypes.length; i++) {
                var checkType = this.triggerTypes[i] || 0;
        ...
        SubSkillData.prototype.doEffectsOnTargets = function (skillInfo, atker, checkType, checkSubType, checkParam, checkProb, effects, prtSkillId, extParam) {
            var tempProb = 0;
            for (var i = 0; i < this.targets.length; i++) {
                var target = this.targets[i];
        ...
        SubSkillData.prototype.modifyProb = function (atker, tag, effects) {
            ...
            if (tabeff && tabeff.type == iface.tb_prop.effectTypeKey.addBuff) {
                ...
                if (tabbuff && tabbuff.type == iface.tb_prop.buffTypeKey.dizzy) {
                    var atkdizzy = atker.isArtifact() ? 0 : atker.getAttr(iface.tb_prop.attrTypeKey.dizzy, tag.getObjId());
                    var tagimmuDizzy = tag.isArtifact() ? 0 : tag.getAttr(iface.tb_prop.attrTypeKey.immuDizzy, atker.getObjId());
                    var temp = 1 + atkdizzy - tagimmuDizzy;
        ...
        SubSkillData.prototype.checkTrigger = function (atker, target, checkSubType, checkParam, extParam) {
            if (atker.isArtifact())
                return true;
        ...
        SubSkillData.prototype.getSpeEftId = function (atker, target, checkType, checkParam, effects, idx) {
            var effectId = 0;
        ...
        SubSkillData.prototype.targetDamageOpt = function (atker, skillInfo, prtSkillId) {
        ...
        SubSkillData.prototype.doDamage = function (skillInfo, atker, prtSkillId) {
            if (!this.checkUseNum()) {
                return;
            }
            ...
            if (this.mainSkill == 1) {
                ...
                if (!atker.scene.isBackAtk) {
                    ...
                    for (var j = 0; j < this.targets.length; j++) {
                        var tag = this.targets[j];
                        tag.setAnger(tag.anger + anger_effect[1], prtSkillId);
                    }
                }
            }
        };
        SubSkillData.prototype.getExtDamage = function (atker, target) {
            var extraDmg = 0;
            if (this.targetExtraDmg.length > 0) {
        ...
        SubSkillData.prototype.doExtDamage = function (extdmg, atker, target) {
        ...
        SubSkillData.prototype.doDamageOnTarget = function (skillInfo, atker, target, bHit) {
            ...
            var atker_atk = atker.getAttr(iface.tb_prop.attrTypeKey.atk, target.getObjId());
            var atker_crit = atker.getAttr(iface.tb_prop.attrTypeKey.crit, target.getObjId());
            var atker_critDmg = atker.getAttr(iface.tb_prop.attrTypeKey.critDmg, target.getObjId());
            var target_def = target.getAttr(iface.tb_prop.attrTypeKey.def, atker.getObjId());
            var target_dmgRate = target.getAttr(iface.tb_prop.attrTypeKey.dmgRate, atker.getObjId());
            var target_delDmg = target.getAttr(iface.tb_prop.attrTypeKey.subDmg, atker.getObjId());
            var atker_addDmg = atker.getAttr(iface.tb_prop.attrTypeKey.addDmg, target.getObjId());
            ...
            if (atker.scene.defType === iface.tb_prop.battleObjTypeKey.god) {
                var pvppara = tb.TB_game_set.get_TB_game_setById(1).pvp_para;
                dmgValue *= pvppara;
        ...
        SubSkillData.prototype.doDmgSputter = function (atker, target, dmgValue, skillInfo) {
        ...
        SubSkillData.prototype.hasRebornEffect = function () {
            for (var i = 0; i < this.effects.length; i++) {
                var effs = this.effects[i];
                for (var j = 0; j < effs.length; j++) {
                    var efftb = tb.TB_effect.get_TB_effectById(effs[j]);
    ...
    battle.SubSkillData = SubSkillData;
})(battle || (battle = {}));
