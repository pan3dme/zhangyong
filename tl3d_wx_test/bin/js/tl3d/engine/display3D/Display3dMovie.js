    ...
    var Display3dMovie = /** @class */ (function (_super) {
        ...
        */
        ...
        Display3dMovie.prototype.randomPlayAction = function () {
            ...
            var idx = random(keys.length);
            var action = keys[idx];
            this.play(action, 2);
        };
        Display3dMovie.prototype.play = function ($action, $completeState, needFollow, cb) {
        ...
        Display3dMovie.prototype.updateFrame = function (t) {
            ...
            this._curentFrame = float2int(this._actionTime / (tl3d.Scene_data.frameTime * 2));
            if (this._curentFrame >= animData.matrixAry.length) {
                //动作播放结束
                if (this._completecb) {
                    this._completecb();
