...
function doEffects(skillInfo, atker, target: battle.BattleObj, effects, skillId, isHit: boolean, after: boolean = false, extParam?) {
...
function doOneEffect(skillInfo, atker: battle.BattleObj, target: battle.BattleObj, effectId, skillId, isHit: boolean, after: boolean = false, extParam?) {
    logfight("atker %d target %d skillId %d, effectId %d", atker.getObjId(), target.getObjId(), skillId, effectId);
    var tbConfig: tb.TB_effect = tb.TB_effect.get_TB_effectById(effectId);
    if (!tbConfig) {
    ...
    switch (type) {
        ...
        case iface.tb_prop.effectTypeKey.chgHp:
            if (!target.isArtifact()) {
                ...
                let curskill = tb.TB_skill.get_TB_skillById(skillId);
...
function doHitObj(atker, target: battle.BattleObj, dmgInfo, skillInfo, skillId, bCrit?: boolean, isShare?: boolean, triggerDeadly?: boolean) {
...
function updateObjHp(atker, target: battle.BattleObj, value: number, skillId, bCrit, triggerDeadly?: boolean) {
...
function pushDominantTarget(atker: battle.BattleObj, targets, objs) {
...
function pushTarget(targets, objs, num, filterFunc, sortFunc, filterArr?: any) {
    if (num <= 0) {
        return;
    }
...
function isInFilter(filterArr, obj): boolean {
...
function modifyPasvAttr(tempskills) {
    let tempskill;
    let tempskilltb: tb.TB_skill;
    let tempsubskilltb: tb.TB_sub_skill;
    let tempeffect: tb.TB_effect;
    let valtype: number;
    let attrkey: number;
    let valpre: number;
    let fixed = {};
