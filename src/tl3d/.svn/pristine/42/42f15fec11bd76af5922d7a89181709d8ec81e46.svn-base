...
module tl3d {
    ...
    export class Dis2DUIContianerPanel extends Dis2DUIContianerBase {
        ...
        private initData($classVo: any, $rect: Rectangle, $num: number): void {
            ...
            this.voRect = $rect;
            this.textureRect = new Rectangle(0, 0, Math.pow(2, Math.ceil(Math.log($rect.width) / Math.log(2))), Math.pow(2, Math.ceil(Math.log($rect.height * this.voNum) / Math.log(2))));

            this._baseRender.uiAtlas = new UIAtlas();
            var $uiAtlas: UIAtlas = this._baseRender.uiAtlas;
            $uiAtlas.configData = new Array();
            $uiAtlas.ctx = UIManager.getInstance().getContext2D(this.textureRect.width, this.textureRect.height, false);
            $uiAtlas.textureRes = TextureManager.getInstance().getCanvasTexture($uiAtlas.ctx);
            this.makeBaseUi($classVo);
        }

        public textureRect: Rectangle;//纹理尺寸
        public voNum: number;//最大同屏数量
        public voRect: Rectangle;//单元尺寸
        protected _uiItem: Array<Disp2DBaseText>;
        protected _lostItem: Array<tl3d.baseMeshVo>;//没有上传成功的
        ...
        protected makeBaseUi($classVo: any): void {
            ...
            for (var i: number = 0; i < this.voNum; i++) {
    ...
    export class Dis2DUIFram extends Dis2DUIContianerPanel {
        public constructor($classVo: any, $rect: Rectangle, $num: number) {
        ...
        protected makeBaseUi($classVo: any): void {
            ...
            for (var i: number = 0; i < this.voNum; i++) {
                ...
                $disp2DBaseText.textureStr = "id" + i;
                $uiAtlas.configData.push($uiAtlas.getObject($disp2DBaseText.textureStr, 0, i * this.voRect.height, this.voRect.width, this.voRect.height, this.textureRect.width, this.textureRect.height));
                $disp2DBaseText.ui = <UICompenent>this._baseRender.createFrame($disp2DBaseText.textureStr);
                $disp2DBaseText.ui.width *= 1.0;
                $disp2DBaseText.ui.height *= 1.0;
            }
        }
