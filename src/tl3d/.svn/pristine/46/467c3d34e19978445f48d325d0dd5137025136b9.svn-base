...
function hexToArgb(expColor: number, is32: boolean = true, color: tl3d.Vector3D = null): tl3d. Vector3D {
    if (!color) {
...
function hexToArgbNum(expColor: number, is32: boolean = true, color: tl3d.Vector3D = null): tl3d. Vector3D {
...
function strokeFilter(ctx: CanvasRenderingContext2D, width: number, height: number, color: number): void {
    var colorVec: tl3d.Vector3D = hexToArgb(color);
 ...
 * 不包括max
 */
function getRandomNumAssgin($much: number, $min: number, $max: number) {
    ...
    for (var i = 0; i < $much; i++) {
        if ($max > i) {
            let randomid: number = random(ary.length);
...
function getZipByte($byte: tl3d.TLByteArray): tl3d. TLByteArray {
