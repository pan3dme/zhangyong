module tl3d {
    export class CombineParticle extends EventDispatcher {
        ...
        public setDataByte(byte: TLByteArray): void {
            byte.position = 0;
        ...
        public updateTime(t: number): void {
            ...
            if (this._time >= this._maxTime) {
                if(this.hasEventListener(BaseEvent.COMPLETE)){ //优化没有监听事件就可以不用抛出
        ...
        public reset(): void {
            ...
            this._time = 0;
            for (var i: number = 0; this._displayAry && i < this._displayAry.length; i++) {
